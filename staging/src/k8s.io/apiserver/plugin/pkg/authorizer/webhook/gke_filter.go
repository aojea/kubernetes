...
func NewGkeApiserverWebhookAuthorizer(config *rest.Config, version string, authorizedTTL, unauthorizedTTL time.Duration, retryBackoff wait.Backoff) (RuleResolverAuthorizer, error) {
	// Create regular WebhookAuthorizer
	webhookAuthorizer, err := New(config, version, authorizedTTL, unauthorizedTTL, retryBackoff)
	if err != nil {
		return nil, err
	}
