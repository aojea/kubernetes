...
func TestNewGkeApiserverWebhookAuthorizer(t *testing.T) {
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			config, err := webhookutil.LoadKubeconfig(fmt.Sprintf("testdata/%s", test.file), nil)
			if err != nil {
				t.Errorf("error occured loading config: %v", err)
				return
			}
			authorizer, err := NewGkeApiserverWebhookAuthorizer(config, "v1", time.Second, time.Second, wait.Backoff{})
			if err != nil {
				t.Errorf("error occured: %v", err)
				return
			}
