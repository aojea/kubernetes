...
func ValidateServiceCIDR(cidrConfig *networking.ServiceCIDR) field.ErrorList {
	...
	if len(cidrConfig.Spec.CIDRs) == 0 {
		allErrs = append(allErrs, field.Invalid(fieldPath, cidrConfig.Spec, "at least one CIDR required"))
		return allErrs
	}

	if len(cidrConfig.Spec.CIDRs) > 2 {
		allErrs = append(allErrs, field.Invalid(fieldPath, cidrConfig.Spec, "max two CIDRs"))
		return allErrs
	}
	...
	if len(cidrConfig.Spec.CIDRs) == 2 {
		...
		if err != nil || !isDual {
			allErrs = append(allErrs, field.Invalid(fieldPath, cidrConfig.Spec, "must be one CIDR from each IP family, i.e 192.168.0.0/24 and 2001:db8::/64"))
			return allErrs
		}
	}
