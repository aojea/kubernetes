...
EOF
  ...
  # Create GKEWarden authz config file, if its env variable is set. The env
  # variable is renamed from GKEAUTOPILOT_AUTHZ_KUBECONFIG to
  # GKEWARDEN_AUTHZ_KUBECONFIG, so both are examined to avoid synchronization
  # gaps.
  #
  # TODO(b/243163006) cleanup examining for GKEAUTOPILOT_AUTHZ_KUBECONFIG env
  # var once the old env variable is phased out.

  if [ -n "${GKEAUTOPILOT_AUTHZ_KUBECONFIG:-}" ]; then
    ...
    # Create webhook kubeconfig file off of $GKEAUTOPILOT_AUTHZ_KUBECONFIG env var
    cat > "/etc/srv/kubernetes/gke-warden-authz.config" <<EOF
$GKEAUTOPILOT_AUTHZ_KUBECONFIG
EOF

  elif [ -n "${GKEWARDEN_AUTHZ_KUBECONFIG:-}" ]; then
    echo "Enabling GKEWarden authz plugin"
