...
func TestMultiMasterNodePortAllocation(t *testing.T) {
	// number of node port services to be created
	numNodePortSvcs := 50
	// number of repetitions to create and delete services
	numReps := 20
	...
	// create 2 api servers and one client for each
	for i := 0; i < 2; i++ {
		// start master count api server
	...
	t.Logf("creating %d nodePort services", numNodePortSvcs)
	for i := 0; i < numNodePortSvcs; i++ {
		if _, err := clientAPIServers[0].CoreV1().Services(metav1.NamespaceDefault).Create(context.TODO(), svc(i), metav1.CreateOptions{}); err != nil {
			t.Error(err)
		}
	}
	...
	for i := 0; i < numReps; i++ {
		...
		t.Logf("creating nodePort service %v", serviceObject)
		...
		t.Logf("deleting nodePort service %s", serviceObject.ObjectMeta.Name)
