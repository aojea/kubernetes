...
func (at *affinityTracker) recordHost(host string) {
	...
	for _, h := range hosts {
		at.hostTrace = append(at.hostTrace, strings.TrimSpace(h))
	}
	framework.Logf("Received response from host: %s", hosts)
}
...
var _ = SIGDescribe("Services", func() {
	...
	*/
	...
	// [LinuxOnly]: Windows does not support session affinity.
	ginkgo.It("should have session affinity timeout work for  for NodePort service  [LinuxOnly]", func() {
...
func execAffinityTestForSessionAffinityTimeout(f *framework.Framework, cs clientset.Interface, svc *v1.Service) {
	...
	svcSessionAffinityTimeout := int32(AffinityConfirmCount / 4)
